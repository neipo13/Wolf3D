...
namespace Nez
{
	public static class Input
	{
		...
		public static void Update()
		{
			...
			if (MouseLockState == MouseLockState.Locked)
			{
				//keep setting the mouse cursor to 0 and only track deltas from 0
				var w = Core.GraphicsDevice.Viewport.Width / 2;
				var h = Core.GraphicsDevice.Viewport.Height / 2;
				Mouse.SetPosition(w, h);
				// also update the previous mouse state to 0 so that our delta is from 0 and not from wherever we had moved from last time
				_previousMouseState = new MouseState(
					w,
					h,
		...
		private static MouseLockState _MouseLockState = MouseLockState.Unlocked;
		public static MouseLockState MouseLockState
		{
			get { return _MouseLockState; }
			set
			{
				_MouseLockState = value;
				if (value == MouseLockState.Unlocked)
				{
					SDL_SetWindowGrab(Core._instance.Window.Handle, false);
				}
				else if (value == MouseLockState.Locked)
				{
					//keep setting the mouse cursor to 0 and only track deltas from 0
					var w = Core.GraphicsDevice.Viewport.Width / 2;
					var h = Core.GraphicsDevice.Viewport.Height / 2;
				...
				else if (value == MouseLockState.Confined)
				{
					SDL_SetWindowGrab(Core._instance.Window.Handle, true);
				}
			}
		}
