...
namespace Nez.Sprites
{
	...
	public class SpriteRenderer : RenderableComponent
	{
				...
				if (_areBoundsDirty)
				{
					if (_sprite != null)
						_bounds.CalculateBounds(Entity.Transform.Position, _localOffset, _origin,
							Scale, Entity.Transform.Rotation, _sprite.SourceRect.Width,
		...
		{
			get => new Vector2(_origin.X / Width * Scale.X,
				_origin.Y / Height * Scale.Y);
			set => SetOrigin(new Vector2(value.X * Width / Scale.X,
				value.Y * Height / Scale.Y));
		}
		...
		public bool FollowsParentEntityScale = true;
		private Vector2 _scale;
		public Vector2 Scale
		{
			get
			{
				if (FollowsParentEntityScale)
					return Entity.Scale;
				return _scale;
			}
			set { _scale = value; FollowsParentEntityScale = false; }
		}
		...
		{
			SetOrigin(new Vector2(value.X * Width / Scale.X,
				value.Y * Height / Scale.Y));
			return this;
		}
		...
		public override void Render(Batcher batcher, Camera camera)
		{
			batcher.Draw(Sprite, Entity.Transform.Position + LocalOffset, Color,
				Entity.Transform.Rotation, Origin, Scale, SpriteEffects, _layerDepth);
		}
	}
}